...
def _meshgrid(height, width, fp):
    ...
    r = tf.pow(d, 2) * tf.log(d+ 1e-6) # [n, nx*ny, h*w] ADDED D COEFFICIENT HERE
...
def perform_interpolation(im, x_s, y_s, out_size):
...
def parameter_affine(affines, T):
    deformable_part = T[:,:,3:]
   
    trans_part = tf.expand_dims(affines[:,0:2,2],dim=-1) 
    xy_part = affines[:,0:2,0:2]
    affine_part = tf.concat([trans_part, xy_part], axis=-1)
   
    new_T = tf.concat([affine_part, deformable_part], axis=-1)
    
    return new_T

def get_parameter_T(affines, cp, nx, ny):
    original_T, fp = _solve_system(cp, nx, ny)
    new_T = parameter_affine(affines, original_T)
    return new_T, fp
